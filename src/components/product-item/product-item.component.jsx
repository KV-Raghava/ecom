...
function ProductItem({ product }) {
  const { title, price, rating, description, id } = product;
  ...
  const addProductToCart = () => {
    dispatch(actions.addToCart({id:product.id}));
  ...
  const handelSave = () => {
    ...
    dispatch(actions.editProduct({'id':id, 'newValues':newValues}));
    toast("Product Updated");
    setBeingEdited(false);
  };

  const handelDelete = () => {
    dispatch(actions.deleteProduct({'id':product.id}));
